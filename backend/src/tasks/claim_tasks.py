...
from models.insurance import BaseInsurance

@shared_task
def validate_claim_task(claim_id):
    try:
        ...
        if Claim.objects.filter(file_hash=file_hash).exclude(id=claim.id).exists():
            claim.status = Claim.Status.REJECTED
            claim.review_notes = "Duplicate invoice upload detected."
            claim.save()
            return "REJECTED: Duplicate hash"

        # 3. Validate coverage period
        insurance = claim.insurance
        if not (insurance.coverage_start <= claim.date_of_event <= insurance.coverage_end):
            claim.status = Claim.Status.REJECTED
        ...
        if 'claim' in locals():
            claim.review_notes = f"System Error during validation: {str(e)}"
